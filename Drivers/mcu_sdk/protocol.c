...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_CONDITION_1, DP_TYPE_BOOL},
  {DPID_CONDITION_2, DP_TYPE_BOOL},
  {DPID_CONDITION_3, DP_TYPE_BOOL},
  {DPID_CONDITION4, DP_TYPE_VALUE},
  {DPID_CONDITION_5, DP_TYPE_VALUE},
  {DPID_CONDITION_6, DP_TYPE_VALUE},
  {DPID_CONDITION_7, DP_TYPE_ENUM},
  {DPID_CONDITION_8, DP_TYPE_ENUM},
  {DPID_CONDITION_9, DP_TYPE_ENUM},
};
...
void uart_transmit_output(unsigned char value)
{
    ...
    uart_value = value;
	HAL_UART_Transmit(&huart2, &uart_value, 1, 1000 );
 ...
 */
// unsigned long CurrentTemperature = 0;
//unsigned long CurrentHumidity = 0;
void all_data_update(void)
{
    //#error "请在此处理可下发可上报数据及只上报数据示例,处理完成后删除该行"
    
    //此代码为平台自动生成，请按照实际数据修改每个可下发可上报函数和只上报函数
    //mcu_dp_value_update(DPID_TEMP_CURRENT,CurrentTemperature); //VALUE型数据上报;
    //mcu_dp_value_update(DPID_HUMIDITY_VALUE,CurrentHumidity); //VALUE型数据上报;
    ...
    mcu_dp_bool_update(DPID_CONDITION_1,当前状态1); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_CONDITION_2,当前状态2); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_CONDITION_3,当前状态3); //BOOL型数据上报;
    mcu_dp_value_update(DPID_CONDITION4,当前状态4); //VALUE型数据上报;
    mcu_dp_value_update(DPID_CONDITION_5,当前状态5); //VALUE型数据上报;
    mcu_dp_value_update(DPID_CONDITION_6,当前状态6); //VALUE型数据上报;
    mcu_dp_enum_update(DPID_CONDITION_7,当前状态7); //枚举型数据上报;
    mcu_dp_enum_update(DPID_CONDITION_8,当前状态8); //枚举型数据上报;
    mcu_dp_enum_update(DPID_CONDITION_9,当前状态9); //枚举型数据上报;
...
/*****************************************************************************
函数名称 : dp_download_condition_1_handle
功能描述 : 针对DPID_CONDITION_1的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_condition_1_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    unsigned char condition_1;
    
    condition_1 = mcu_get_dp_download_bool(value,length);
    if(condition_1 == 0) {
        //开关关
    }else {
        //开关开
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_CONDITION_1,condition_1);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_condition_2_handle
功能描述 : 针对DPID_CONDITION_2的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_condition_2_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    unsigned char condition_2;
    
    condition_2 = mcu_get_dp_download_bool(value,length);
    if(condition_2 == 0) {
        //开关关
    }else {
        //开关开
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_CONDITION_2,condition_2);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_condition_3_handle
功能描述 : 针对DPID_CONDITION_3的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_condition_3_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    unsigned char condition_3;
    
    condition_3 = mcu_get_dp_download_bool(value,length);
    if(condition_3 == 0) {
        //开关关
    }else {
        //开关开
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_CONDITION_3,condition_3);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_condition4_handle
功能描述 : 针对DPID_CONDITION4的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_condition4_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long condition4;
    
    condition4 = mcu_get_dp_download_value(value,length);
    /*
    //VALUE类型数据处理
    ...
    */
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_value_update(DPID_CONDITION4,condition4);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_condition_5_handle
功能描述 : 针对DPID_CONDITION_5的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_condition_5_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long condition_5;
    
    condition_5 = mcu_get_dp_download_value(value,length);
    /*
    //VALUE类型数据处理
    ...
    */
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_value_update(DPID_CONDITION_5,condition_5);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_condition_6_handle
功能描述 : 针对DPID_CONDITION_6的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_condition_6_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long condition_6;
    
    condition_6 = mcu_get_dp_download_value(value,length);
    /*
    //VALUE类型数据处理
    ...
    */
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_value_update(DPID_CONDITION_6,condition_6);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_condition_7_handle
功能描述 : 针对DPID_CONDITION_7的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_condition_7_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char condition_7;
    
    condition_7 = mcu_get_dp_download_enum(value,length);
    switch(condition_7) {
        case 0:
        break;
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_enum_update(DPID_CONDITION_7, condition_7);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_condition_8_handle
功能描述 : 针对DPID_CONDITION_8的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_condition_8_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char condition_8;
    
    condition_8 = mcu_get_dp_download_enum(value,length);
    switch(condition_8) {
        case 0:
        break;
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_enum_update(DPID_CONDITION_8, condition_8);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        ...
        break;
        case DPID_CONDITION_1:
            //状态1处理函数
            ret = dp_download_condition_1_handle(value,length);
        break;
        case DPID_CONDITION_2:
            //状态2处理函数
            ret = dp_download_condition_2_handle(value,length);
        break;
        case DPID_CONDITION_3:
            //状态3处理函数
            ret = dp_download_condition_3_handle(value,length);
        break;
        case DPID_CONDITION4:
            //状态4处理函数
            ret = dp_download_condition4_handle(value,length);
        break;
        case DPID_CONDITION_5:
            //状态5处理函数
            ret = dp_download_condition_5_handle(value,length);
        break;
        case DPID_CONDITION_6:
            //状态6处理函数
            ret = dp_download_condition_6_handle(value,length);
        break;
        case DPID_CONDITION_7:
            //状态7处理函数
            ret = dp_download_condition_7_handle(value,length);
        break;
        case DPID_CONDITION_8:
            //状态8处理函数
            ret = dp_download_condition_8_handle(value,length);
        break;
